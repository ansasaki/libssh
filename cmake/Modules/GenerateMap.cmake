...
#
# Generates a symbols version script using the smap tool.
# This script is run in build time to use the correct command depending on the
# existence of the file provided ``CURRENT_MAP``.
#
# If the file exists, the ``smap update`` subcommand is used to update the
# existing map. Otherwise, the ``smap new`` subcommand is used to create a new
# map file.
#
# If the file provided int ``CURRENT_MAP`` exists, it is copied to the
# ``OUTPUT_PATH`` before updating.
# This is required because ``smap`` do not generate output if no symbols were
...
#
# ``SMAP_EXECUTABLE``:
#   Required, expects the path to the ``smap`` tool.
#
# Optionally defined variables
# ----------------------------
#
# ``FINAL``:
#   If defined, will mark the modified release as final, preventing later
#   changes using ``smap``.
#
# ``BREAK_ABI``:
...
if (NOT SMAP_EXECUTABLE)
    message(SEND_ERROR "SMAP_EXECUTABLE not defined")
endif()
...
if (EXISTS ${CURRENT_MAP})
    ...
    execute_process(
      COMMAND
        ${CMAKE_COMMAND} -E copy ${CURRENT_MAP} ${OUTPUT_PATH}
      COMMAND
        ${SMAP_EXECUTABLE} update ${ARGS_LIST}
